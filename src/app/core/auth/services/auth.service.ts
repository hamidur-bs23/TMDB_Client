...
import {
  ...
  ICreateSessionWithLoginCredentialsRequest, ILogoutSessionResponse
} from "../models/models";
...
export class AuthService {
  ...
  CreateRequestToken() {
    ...
    return this.httpClient.get<IAuthRequestTokenResponse>(url)
      .pipe(
        map((response) => {
          if (response.success) {
  ...
  CreateSession() {
    debugger;
    const url: string = `${environment.api_tmdb_auth.api_base_url}/3/authentication/session/new?api_key=${this.getApiKey()}`;
    const requestToken: string = localStorage.getItem('user-request-token') ?? '';
  ...
  CreateSessionWithUserCredentials(username: string, password: string) {
    ...
    const url = `${environment.api_tmdb_auth.api_base_url}/3/authentication/token/validate_with_login?api_key${this.getApiKey()}`;

    this.httpClient.post<IAuthRequestTokenResponse>(url, body)
      .pipe(
        map((response) => {
          // debugger;
          if (response.success) {
  ...
  // ver: 3
  Logout() {
    const url = `${environment.api_tmdb_auth.api_base_url}/3/authentication/session?api_key=${this.getApiKey()}`;
    ...
    if (sessionId) {
      ...
      return this.httpClient.delete<ILogoutSessionResponse>(url, {
        body: {
